...
Public Class SeniorMgrAppraisal
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            UserEN.Formid = "frm_hr_SApproval"
        ...
        End If
    End Sub
    Private Sub PageLoadBind(ByVal objen As SeniorMgrAppraisalEN)
    ...
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Else
            ...
            If AddUDOUpdate("Save") = True Then
                ...
                panelnew.Visible = False
            Else
                dbcon.strmsg = "alert('Senior Manager Appraisal Request Approved failed...')"
                mess(dbcon.strmsg)
            End If
        End If
    End Sub
    Private Function AddUDOUpdate(ByVal strchoice As String) As Boolean
        Try
            ...
            objen.BusinessRemarks = txtBSrremarks.Text.Trim()
            objen.PeopleRemarks = txtPSrremarks.Text.Trim()
            objen.CompRemarks = txtCSrremarks.Text.Trim()
            ...
            If strchoice = "SaveSubmit" Then
                objen.CheckStatus = "Y"
                objen.Status = "SM"
                objDA.UpdatesrMgrAppHeader(objen)
                dbcon.strmsg = SendMailforAppraisal(txtAppno.Text.Trim(), "HA", Session("SAPCompany"), "HA")
            Else
                objen.CheckStatus = "N"
                objen.Status = "LM"
                objDA.UpdatesrMgrAppHeader(objen)
    ...
    Protected Sub btnclose_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnclose.Click
        ...
        btnnew.Visible = False
    End Sub
    Private Sub ObjectiveBind(ByVal objen As SeniorMgrAppraisalEN)
    ...
    Protected Sub lbtndocnum_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        End If
    End Sub
    Private Sub PopulateAppraisal(ByVal objen As SeniorMgrAppraisalEN)

        ObjectiveBind(objen)
        If objen.EmpId <> "" Then
            dbcon.ds = objBL.PopulateEmployee(objen)
            If dbcon.ds.Tables(0).Rows.Count <> 0 Then
                txtAppno.Text = dbcon.ds.Tables(0).Rows(0)("DocEntry").ToString()
                txtreqdate.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_Date").ToString()
                txtempid.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_empID").ToString()
                txtempname.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_EmpName").ToString()
                ddlstatus.SelectedIndex = ddlstatus.Items.IndexOf(ddlstatus.Items.FindByText(dbcon.ds.Tables(0).Rows(0)("U_Z_WStatus").ToString()))
                If dbcon.ds.Tables(0).Rows(0)("U_Z_Period").ToString() <> "" Then
                    objen.BusinessRemarks = dbcon.ds.Tables(0).Rows(0)("U_Z_Period").ToString()
                    ddlperiod.SelectedItem.Text = objen.BusinessRemarks
                End If
                lblPeriodfrom.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_PerFrom").ToString()
                lblperioTo.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_PerTo").ToString()

                txtsBremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_BSelfRemark").ToString()
                txtsPremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_PSelfRemark").ToString()
                txtsCremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_CSelfRemark").ToString()

                txtBSrremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_BSMrRemark").ToString()
                txtPSrremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_PSMrRemark").ToString()
                txtCSrremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_CSMrRemark").ToString()

                txtBLmremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_BMgrRemark").ToString()
                txtPLmremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_PMgrRemark").ToString()
                txtCLmremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_CMgrRemark").ToString()

                txtCHrremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_CHrRemark").ToString()
                txtPHrremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_PHrRemark").ToString()
                txtBCHrremarks.Text = dbcon.ds.Tables(0).Rows(0)("U_Z_BHrRemark").ToString()

                EnableDisable(dbcon.ds.Tables(0).Rows(0)("U_Z_WStatus").ToString(), dbcon.ds.Tables(0).Rows(0)("U_Z_GStatus").ToString())
            End If
        End If
    End Sub
    Private Sub EnableDisable(ByVal strStatus As String, ByVal grvstatus As String)
        ...
        If strStatus <> "HR Approved" And strStatus <> "Draft" And strStatus <> "SelfApproved" And grvstatus = "-" Then
            ...
            txtBSrremarks.ReadOnly = False
            btnsavesubmit.Visible = True
        Else
            ddlstatus.Enabled = False
            ddlperiod.Enabled = False
            txtBSrremarks.ReadOnly = True
            btnsubmit.Visible = False
    ...
    Protected Sub ddlsmgrfrate_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
    ...
    Protected Sub ddlCompsmgrfrate_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim gv As GridViewRow = CType((link.Parent.Parent), GridViewRow)
        Dim i As Integer = gv.RowIndex
        Dim ddl As DropDownList = DirectCast(grdCompetenceview.Rows(i).FindControl("ddlcompsmgrrate"), DropDownList)
    ...
    Private Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        Else
            ...
            If objen.StrType <> "" Then
                Empid = "T0.""U_Z_EmpId""='" & objen.StrType & "'"
            Else
                Empid = "1=1"
            End If
            ...
            If objen.Period <> 0 Then
                Period = "T0.""U_Z_Period""='" & ddlsrPeriod.SelectedValue & "'"
            Else
                Period = "1=1"
            End If
    ...
    Private Sub LnkViewall_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LnkViewall.Click
        ...
        PageLoadBind(objen)
    End Sub
    Public Function SendMailforAppraisal(ByVal strDocEntry As String, ByVal StrType As String, ByVal SAPCompany As SAPbobsCOM.Company, ByVal strLevel As String) As String
    ...
    Private Function SendMailforUsers(ByVal mailServer As String, ByVal mailPort As String, ByVal mailId As String, ByVal mailpwd As String, ByVal mailSSL As String, ByVal toId As String, ByVal ccId As String, ByVal mType As String, ByVal path As String, ByVal DocEntry As String, ByVal Name As String, ByVal SAPCompany As SAPbobsCOM.Company, Optional ByVal Period As String = "") As String
        Try
            ...
            ElseIf mType = "LA" Then
                oRecordSet.DoQuery("Select T0.""U_Z_PerFrom"" ,T0.""U_Z_PerTo"",T1.U_Z_Empid from ""@Z_HR_PERAPP"" T0 JOIN [@Z_HR_OSEAPP] T1 on T1.U_Z_Period=T0.U_Z_PerCode where T1.DocEntry='" & DocEntry & "'")
                Dim strPeriod As String = "Period From: " & oRecordSet.Fields.Item("U_Z_PerFrom").Value & "    " & "Period To: " & oRecordSet.Fields.Item("U_Z_PerTo").Value
                strQuery = "SELECT isnull(T0.firstName,'') +' '+ isnull(T0.lastName,'') as 'EmpName',T1.userId from OHEM T0 JOIN OHEM T1 ON T0.Manager=T1.empID where T0.empID IN (SELECT manager FROM OHEM WHERE empID =" & oTemp.Fields.Item("U_Z_EmpId").Value & ")"
                oTemp.DoQuery(strQuery)
                If oTemp.RecordCount > 0 Then
                    oTest.DoQuery("Select * from [@Z_HR_OWEB]")
                    Dim strESSLink As String = ""
